...
client = commands.Bot(description="Bot prefix is el!", command_prefix=commands.when_mentioned_or("el!" ), pm_help = True)
...
async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for el!help', type=2))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='for el!help2', type=2))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='for el!help3', type=2))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='created by DP♡#1471', type=2))
        await asyncio.sleep(5)
...
async def on_ready():
     ...
     print('.......')
     print('created by DP♡#1471')
     client.loop.create_task(status_task())

def is_owner(ctx):
     return ctx.message.author.id in ["456296736613335050","429301779981795338"]
...
async def dm(ctx, user: discord.Member, *, msg: str):
   if user is None or msg is None:
       await client.say('Invalid args. Use this command like: ``el!dm @user message``')
   if ctx.message.author.server_permissions.kick_members == False:
       await client.say('**You do not have permission to use this command**')
       return
   else:
...
async def getuser(ctx, role: discord.Role = None):
    if role is None:
        await client.say('Please tag a role to get users having it. Example- ``el!getuser @role``')
        return
    if ctx.message.author.server_permissions.kick_members == False:
       await client.say('**You do not have permission to use this command**')
       return
...
async def userinfo(ctx, user: discord.Member=None):
    if user is None:
      await client.say('Please tag a user to get user information. Example- ``el!userinfo @user``')
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def rolecolor(ctx, role:discord.Role=None, value:str=None):
    if discord.utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
        await client.say("Use this command like ``el!rolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
    if value is None:
        await client.say("Use this command like ``el!rolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
...
async def warn(ctx, userName: discord.User=None,*, message:str=None):
    if userName is None:
      await client.say('Please tag a person to warn user. Example- ``el!warn @user <reason>``')
      return
    else:
...
async def announce(ctx, channel: discord.Channel=None, *, msg: str=None):
    ...
    if channel is None or msg is None:
        await client.say('Invalid args. Use this command like ``el!announce #channel text here``')
        return
    else:
...
async def delchannel(ctx, channel: discord.Channel=None):
    if channel is None:
        ...
        await client.send_message(ctx.message.author, "{} channel has been deleted in {}".format(ctx.message.channel.n*ame, ctx.message.server.name))
    else:
        if ctx.message.author.server_permissions.administrator == False:
            await client.say('**You do not have permission to use this command**')
            return
...
async def mute(ctx, member: discord.Member=None, mutetime=None):
    ...
    if member is None:
        ...
        return
    if mutetime is None:
        await client.say('Please specify time i.e. Mention a member to mute with time. Example-``el!mute @user <time in minutes>``')
        return
...
async def unmute(ctx, member: discord.Member=None):
    if member is None:
      await client.say('Please specify member i.e. Mention a member to unmute. Example- ``el!unmute @user``')
    if ctx.message.author.bot:
      return
    else:
      if ctx.message.author.server_permissions.kick_members == False:
...
async def setnick(ctx, user: discord.Member=None, *, nickname=None):
    if user is None:
      await client.say('Please tag a person to change nickname. Example- ``el!setnick @user <new nickname>``')
      return
    else:
...
async def ban(ctx,user:discord.Member=None):
    if user is None:
      await client.say('Please specify a member to ban. Example- ``el!ban @user``')
...
async def rps(ctx, *, message=None):
    ...
    if message is None:
        await client.say('Use it like ``el!rps rock or scissors or paper`` anyone of them to make this command work properly')
...
async def invite():
    ...
    embed.add_field(name = 'bot info', value='Elite is a simple bot to use easy,')
    await client.say(embed=embed)
    await client.delete_message(ctx.message)
...
async def ownerinfo(ctx):
    ...
    embed.set_author(name=" Bot Owner ""kakashi#1696")
    embed.add_field(name="Co-owner: DP♡#1471", value="He coded the bot")
    embed.set_image(url="https://cdn.discordapp.com/attachments/520145083459305492/548223305325215764/images.png")
    embed.add_field(name="bot info", value="Elite is a bot fun and simple to use")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/520145083459305492/548223305325215764/images.png")
    await client.say(embed=embed)


@client.command(pass_context=True)
async def server(ctx):
    await client.say('link to join support server : https://discord.gg/eCPPmSj')
...
async def devkick(ctx,user:discord.Member):
    if user is None:
      await client.say('Please mention a member to kick. Example- ``el!devkick @user``')
    if user.server_permissions.kick_members:
      await client.say('**He is mod/admin and i am unable to kick him/her**')
      return
    else:
