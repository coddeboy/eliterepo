...
client = Bot(description="Here is some commands for you", command_prefix="el!", pm_help = False) #add_your_bot_description_with_prefix_here
...
async def on_ready():
	...
	print('--------')
	print('--------')
	print('Started Elite club') #add_your_bot_name_here
	return await client.change_presence(game=discord.Game(name='Elite club || el!help ')) #add_your_bot_status_here

def is_owner(ctx):
    return ctx.message.author.id == "456296736613335050" #replace_it_with_your_discord_id

def is_owner(ctx):
	return ctx.message.author.id == "456296736613335050"
	#replace_it_with_your_discord_id
...
async def server(ctx):
    ...
    await client.say('https://discord.gg/eCPPmSj')
...
async def info(ctx):
    ...
    await client.say('***THIS BOT IS MADE BY Dp♡#1471 AND IF YOU NEED TO INVITE THIS BOT SO WRITE `el!botinvite` THANKS***')

@client.command(pass_context = True)

async def botinvite(ctx):

    await client.say('https://discordapp.com/api/oauth2/authorize?client_id=548188725834940424&permissions=8&scope=bot')
...
async def avatar(ctx, user: discord.Member=None):
    if user is None:
        ...
        embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/518823417810518016/519049239750508545/Loading.gif') 
...
async def ownerinfo(ctx):
    ...
    embed.set_footer(text="Elite Club")
    embed.set_author(name=" Bot Owner Name- Dp♡#2471, 429301779981795338")
