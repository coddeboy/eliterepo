...
def is_owner(ctx):
    return ctx.message.author.id == "456296736613335050, 429301779981795338"
...
class Modmail(commands.Bot):
    ...
    def token(self):
        ...
        try:
            with open('config.json') as el!:
                config = json.load(f)
                if config.get('TOKEN') == "NTQ4MTg4NzI1ODM0OTQwNDI0.D1Bxsw.oXNEow4jpESHFdXUD28N7_vsv4c":
    ...
    async def get_pre(bot, message):
        '''Returns the prefix.'''
        with open('config.json') as el!:
            prefix = json.load(el!).get('PREFIX')
        return os.environ.get('PREFIX') or prefix or 'el!'

    @staticmethod
    def run_wizard():
        '''Wizard for first start'''
        print('------------------------------------------')
        token = input('NTQ4MTg4NzI1ODM0OTQwNDI0.D1Bxsw.oXNEow4jpESHFdXUD28N7_vsv4c:\n> ')
        print('------------------------------------------')
        data = {
                "TOKEN" : token,
            }
    ...
    def rules_embed(self, prefix):
        ...
        em.add_field(name='Github', value='owm mod mail')
    ...
    def help_embed(self, prefix):
        ...
        cmds = el!'`{prefix}setup [modrole] <- (optional)` - Command that sets up the bot.\n' \
               el!'`{prefix}reply <message...>` - Sends a message to the current thread\'s recipient.\n' \
               el!'`{prefix}close` - Closes the current thread and deletes the channel.\n' \
               el!'`{prefix}disable` - Closes all threads and disables modmail for the server.\n' \
               el!'`{prefix}block` - Blocks a user from using modmail!' \
               el!'`{prefix}unblock` - Unblocks a user from using modmail!'
        ...
        em.add_field(name='Warning', value=warn)
        em.add_field(name='Github', value='own mod mail')
